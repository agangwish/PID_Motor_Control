...
% This function takes a list of "via point" times and positions and
% generates a trajectory (positions as a function of time, in sample
% periods) using either a step trajectory or cubic interpolation.
%
...
%
%   Output:
%       An array ref, each element representing the reference position, in
%       degrees, spaced at time 1/f, where f is the frequency of the
%       trajectory controller. Also plots ref.
%
%   Example usage: ref = genRef([0, 0; 1.0, 90; 1.5, -45; 2.5, 0], 'cubic')
%   Example usage: ref = genRef([0, 0; 1.0, 90; 1.5, -45; 2.5, 0], 'step')
%
%   The via points are 0 degrees at time 0 s; 90 degrees at time 1 s; -45
%   degrees at 1.5 s; and 0 degrees at 2.5 s.
%
%   Note: the first time must be 0, and the first and last velocities
%   should be 0.

MOTOR_SERVO_RATE = 200;     % 200 Hz motion control cycle
...
reflist(1, 1) = 0;          % first time must be 0
for i = 1:numpos
   if (i > 2)
...
% str = sprintf('%d samples at %7.2f Hz taking %5.3 sec', ...
